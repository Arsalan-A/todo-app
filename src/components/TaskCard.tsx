...
const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
  return (
    <div className='flex items-start gap-3 p-4 pt-3 bg-accent-custom h-[72px] rounded-lg text-white '>
      ...
      <AlertDialog>
        <AlertDialogTrigger asChild>
          <button
            className='border-none bg-transparent hover:opacity-50 transition-opacity'
            onClick={() => console.log('delete')}
          >
            <Image src={trash} alt='trash' width={100} height={100} />
          </button>
        </AlertDialogTrigger>
        <AlertDialogContent>
          <AlertDialogHeader>
